...
class PredictionTile extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    List<String> address = prediction.description!.split(', ');
    return ListTile(
      // leading: const Icon(Icons.location_on),
      shape: const UnderlineInputBorder(borderSide: BorderSide(width: 0.075)),
      title: Text(
        address[0],
      ...
      onTap: () {
        ...
        print(prediction.id);
        print(prediction.terms[0].value);
        Navigator.pop(context, {
      ...
      subtitle: Text(
          (address.getRange(1, address.length).toString()).substring(1, address.getRange(1, address.length).toString().length-1),
      ),
    );
  }
}
